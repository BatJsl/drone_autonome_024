...
class CorridorObstacle(WallObstacle):
    ...
    def second_wall(self):
        ...
        """
        x_origin2 = self._width * np.cos(np.radians(self.angle)-np.pi/2) + self._x_origin            # il faut convertir l'angle en radians
        y_origin2 = self._width * np.sin(np.radians(self.angle)-np.pi/2) + self._y_origin            # pour trouver la position initiale
        return WallObstacle(x_origin2, y_origin2, self._dimension, self.angle)
    ...
    def walls_corridor(self):
        ...
        """
        walls_of_corridor = []
        walls_of_corridor.append(self)
        walls_of_corridor.append(self.second_wall())
